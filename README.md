...
5. `safe distance` is the minimum front and back distance for the ego car to take decisions. It is based on the distance a car needs to go from 50 mph to a full stop without violating maximum jerk or acceleration. `V_f = V_i + a*t` In meters assuming an acceleration of ±5 [m/s2] `0 = 22.352 + (-5)*t` requires a time t of 4.47 seconds. Therefore `safe_distance = V_i*t + 1/2*a*t^2` in meters `safe_distance = 22.352*4.47 + 1/2*(-5)*4.47^2 = 49.96 [m]`. Using a dynamic safe distance based on the surrounding vehicle speed rather than a constant requires a more advance prediction step rather than just using real time values as implemented in this repo.  